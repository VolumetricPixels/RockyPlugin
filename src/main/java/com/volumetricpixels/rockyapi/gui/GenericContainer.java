...
public class GenericContainer extends GenericWidget implements Container {
	...
	protected int minWidthCalc = 0, maxWidthCalc = 427, minHeightCalc = 0,
			maxHeightCalc = 240;
	...
	public Container insertChild(int index, Widget child) {
		...
		if (getScreen() != null) {
			...
			getScreen().attachWidget(
					p == Bukkit.getServer().getPluginManager()
							.getPlugin("Spout") ? getPlugin() : p, child);
		}
		updateSize();
		deferLayout();
		return this;
	...
	public Container updateLayout() {
		if (!recalculating && super.getWidth() > 0 && super.getHeight() > 0
				&& !children.isEmpty()) {
			...
			int availableWidth = auto ? getWidth() : getMinWidth(), availableHeight = auto ? getHeight()
					: getMinHeight();
			// We only layout visible children, invisible ones have zero
			...
			if (type == ContainerType.OVERLAY) {
				...
				for (Widget widget : visibleChildren) {
					int horiz = widget.getMarginLeft()
				...
				newheight = (availableHeight - totalheight)
						/ Math.max(1, vcount);
				// Deal with minWidth and minHeight - change newwidth/newheight
				// if needed
				for (Widget widget : visibleChildren) {
					if (!widget.isFixed()) {
						if (type == ContainerType.VERTICAL) {
							if (widget.getMinHeight() > newheight) {
								totalheight += widget.getMinHeight()
										- newheight;
								newheight = (availableHeight - totalheight)
										/ Math.max(1, vcount);
							...
							if (widget.getMinWidth() > newwidth) {
								...
								newwidth = (availableWidth - totalwidth)
										/ Math.max(1, hcount);
			...
			for (Widget widget : visibleChildren) {
				...
				if (!widget.isFixed()) {
					if (auto) {
						widget.setHeight(Math.max(
								widget.getMinHeight(),
								Math.min(
										newheight
												- (this.type == ContainerType.VERTICAL ? 0
														: vMargin), widget
												.getMaxHeight())));
						widget.setWidth(Math.max(
								widget.getMinWidth(),
								Math.min(
										newwidth
												- (this.type == ContainerType.HORIZONTAL ? 0
														: hMargin), widget
												.getMaxWidth())));
					} else {
						widget.setHeight(widget.getMinHeight() == 0 ? newheight
								- vMargin : widget.getMinHeight());
						widget.setWidth(widget.getMinWidth() == 0 ? newwidth
								- hMargin : widget.getMinWidth());
					}
				}
				if (type == ContainerType.VERTICAL) {
			...
			int left = super.getX();
			int top = super.getY();
			if (align == WidgetAnchor.TOP_CENTER
					|| align == WidgetAnchor.CENTER_CENTER
					|| align == WidgetAnchor.BOTTOM_CENTER) {
				left += (super.getWidth() - totalwidth) / 2;
			} else if (align == WidgetAnchor.TOP_RIGHT
					|| align == WidgetAnchor.CENTER_RIGHT
					|| align == WidgetAnchor.BOTTOM_RIGHT) {
				left += super.getWidth() - totalwidth;
			}
			if (align == WidgetAnchor.CENTER_LEFT
					|| align == WidgetAnchor.CENTER_CENTER
					|| align == WidgetAnchor.CENTER_RIGHT) {
				top += (super.getHeight() - totalheight) / 2;
			} else if (align == WidgetAnchor.BOTTOM_LEFT
					|| align == WidgetAnchor.BOTTOM_CENTER
					|| align == WidgetAnchor.BOTTOM_RIGHT) {
			...
			for (Widget widget : visibleChildren) {
				...
				if (type == ContainerType.VERTICAL) {
					top += widget.getHeight() + widget.getMarginTop()
							+ widget.getMarginBottom();
				} else if (type == ContainerType.HORIZONTAL) {
					left += widget.getWidth() + widget.getMarginLeft()
							+ widget.getMarginRight();
				}
			}
			recalculating = false;
	...
	public Container updateSize() {
		if (!recalculating && !isFixed()) {
			recalculating = true; // Prevent us from getting into a loop due to
									// both trickle down and push up
			int minwidth = 0, maxwidth = 0, minheight = 0, maxheight = 0, minhoriz, maxhoriz, minvert, maxvert;
			// Work out the minimum and maximum dimensions for the contents of
			// this container
			for (Widget widget : children) {
				if (widget.isVisible()) {
					if (widget instanceof Container) { // Trickle down to
														// children
						((Container) widget).updateSize();
					}
					minhoriz = maxhoriz = widget.getMarginLeft()
							+ widget.getMarginRight();
					minvert = maxvert = widget.getMarginTop()
							+ widget.getMarginBottom();
					if (widget.isFixed()) {
			...
			if (minwidth != minWidthCalc || maxwidth != maxWidthCalc
					|| minheight != minHeightCalc || maxheight != maxHeightCalc) {
