 ...
 * perform an automatic layout of child elements. This can be used to provide a
 * "group" type container that doesn't change the layout of any children.
 * <p/>
 * Most Widget methods that affect position or layout will be passed down to all
 * children when used on the container itself, that includes setDirty().
 * <p/>
...
public interface Container extends Widget {
	 ...
	 * 
	 * @param index
	 *            The position to insert it, use -1 for append
	 * @param child
	 *            The widget to add
	 * @return Widget
	 */
	...
	/**
	 * Set the automatic layout type for children, triggered by setWidth() or
	 * setHeight()
	 * 
	 * @param type
	 *            ContainerType.VERTICAL, .HORIZONTAL or .OVERLAY
	...
	/**
	 * Force the container to re-layout all non-fixed children. Unless you
	 * specifically need to update the layout at this instant, you should use
	 * use deferLayout() instead. This will re-position and resize all child
	 * elements.
	 * 
	 * @return
	 */
	...
	/**
	 * Automatically call updateLayout during the next onTick. This is
	 * automatically called when anything changes that would affect the
	 * container layout. NOTE: Subclasses should ensure they don't prevent
	 * Container.onTick() from running.
	 * 
	 * @return
	 */
